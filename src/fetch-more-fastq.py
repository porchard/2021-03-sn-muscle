...
def parse_file_name(f):
    RE = '/lab/data/seqcore/{}/parker/Sample_(\d+)-NM-\d+_([a-z])/.*(20|40)K_.*_R(\d+)_.*.fastq.gz'.format(RUN)
    sample, well, nuclei, read = re.match(RE, f).groups()
    return {'sample': sample, 'well': well, 'nuclei': nuclei, 'read': read}

files = [f for f in glob.glob('/lab/data/seqcore/{}/parker/*/*.fastq.gz'.format(RUN)) if '_I1_' not in f]
