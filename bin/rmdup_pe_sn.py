...
def update_read_cache(cur_by_mpos, keep_cache, discard_cache,
                      read_stats, outfile):
    for (mpos, barcode), read_list in list(cur_by_mpos.items()):
        # only keep one read from list with same pos,mate_pos,barcode set
    ...
    for read in infile:
        ...
        if read.qname in keep_cache:
            ...
            if not read.has_tag('CB'):
                sys.stderr.write('Read {} missing barcode tag (CB)!'.format(read.qname))
                sys.exit(1)
            key = (read.mpos, read.get_tag('CB'))
            if key in cur_by_mpos:
                cur_by_mpos[key].append(read)
            else:
                cur_by_mpos[key] = [read]
