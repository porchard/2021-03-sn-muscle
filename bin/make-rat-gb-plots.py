...
bw_files['track_name'] = bw_files.file.map(lambda x: os.path.basename(x).replace('-rn6', '').replace('.bw', ''))
...
rows = 1 + len(bw_files) + len(CHROMHMM_FILES)
height_ratios = [2 for i in range(len(bw_files))] + [1 for i in CHROMHMM_FILES] + [2]
...
ax = axs[0,1]
ax.set_title(f'{REGION} (rat)')

# fill in the bigwig tracks
for row_index, bw_file in enumerate(bw_files.index, 0):
